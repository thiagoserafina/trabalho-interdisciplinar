...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "DELETE FROM funcionarios WHERE id_funcionario = $1 RETURNING *",
      [id]
    );
...
// Rota GET: Listar funcionários por especialidade
router.get("/especialidade/:id_especialidade", async (req, res) => {
  ...
  try {
    const query = `
        ...
        `;
    const values = [id_especialidade];
    const result = await pool.query(query, values);

    if (result.rowCount === 0) {
      return res.status(404).json({
        error: "Nenhum funcionário encontrado para esta especialidade",
      });
    }

    res.status(200).json(result.rows);
  } catch (error) {
    console.error("Erro ao buscar funcionários por especialidade:", error);
    res
      .status(500)
      .json({ error: "Erro ao buscar funcionários por especialidade" });
  }
});
