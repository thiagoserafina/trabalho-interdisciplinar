...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Especialidade adicionada com sucesso', especialidade: result.rows[0] });
    } catch (error) {
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Especialidade atualizada com sucesso', especialidade: result.rows[0] });
    } catch (error) {
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        const result = await pool.query('DELETE FROM especialidades WHERE id_especialidade = $1 RETURNING *', [id]);
        if (result.rowCount === 0) {
            return res.status(404).json({ error: 'Especialidade não encontrada' });
        }
