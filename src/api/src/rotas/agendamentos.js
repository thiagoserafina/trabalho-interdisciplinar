  ...
  try {
    const query = `
        ...
        FROM 
          agendamentos ag
        INNER JOIN 
          alunos al ON ag.id_aluno = al.id_aluno
        INNER JOIN 
          funcionarios func ON ag.id_funcionario = func.id_funcionario
        INNER JOIN
          especialidades esp ON func.id_especialidade = esp.id_especialidade;
      `;
...
router.post("/cancelar-agendamentos", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Agendamentos cancelados para o feriado." });
  } catch (error) {
