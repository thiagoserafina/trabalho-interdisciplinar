...
router.post('/criarusuario', async (req, res) => {
    ...
    try {
     ...
     const query = 'INSERT INTO login (nome, usuario, senha) VALUES ($1, $2, $3)';

     console.log('Usuário criado com sucesso.');
     const values = [nome, usuario, hashedPassword];
     const result = await pool.query(query, values);
     res.status(201).json({ message: 'Usuário criado com sucesso.', login: result.rows[0] });
    } catch (error) {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const query = 'SELECT id_login, usuario, senha FROM login WHERE usuario = $1';
